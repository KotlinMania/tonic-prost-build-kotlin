# Claude Code Project Instructions — tonic-prost-build-kotlin

## Project Overview

This is **tonic-prost-build-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tonic-prost-build`](https://crates.io/crates/tonic-prost-build).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tonic-prost-build/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tonic-prost-build>
