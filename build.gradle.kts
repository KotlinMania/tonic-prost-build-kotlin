...
kotlin {
    ...
    val xcf = XCFramework("TonicProstBuild")

    macosArm64 {
        binaries.framework { baseName = "TonicProstBuild"; xcf.add(this) }
    }
    iosArm64 {
        binaries.framework {
            baseName = "TonicProstBuild"
            isStatic = true
            xcf.add(this)
        }
    }
    ...
    swiftExport {
        moduleName = "TonicProstBuild"
        flattenPackage = "io.github.kotlinmania.tonicprostbuild"
    }

    android {
        namespace = "io.github.kotlinmania.tonicprostbuild"
...
mavenPublishing {
    ...
    coordinates(group.toString(), "tonic-prost-build-kotlin", version.toString())

    pom {
        name.set("tonic-prost-build-kotlin")
        description.set("Kotlin Multiplatform port of hyperium/tonic - Prost build integration for tonic")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/tonic-prost-build-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/tonic-prost-build-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/tonic-prost-build-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/tonic-prost-build-kotlin.git")
        }
    }
}
